...
public class SearchStoreAdapter extends BaseAdapter {
    ...
    private List<SearchStoreBean.SjmsgBean> sjmsg;

    public SearchStoreAdapter(Context context,List<SearchStoreBean.SjmsgBean> sjmsg) {
    ...
    @Override
    public int getCount() {
        return sjmsg.size() == 0 ? 0 : sjmsg.size();
    }

    @Override
    public Object getItem(int position) {
        return sjmsg.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.tv_commodity_name.setText(sjmsg.get(position).getBname());
        holder.tv_comment.setText(sjmsg.get(position).getPlcnt() + "条评论数");
        holder.tv_address.setText(sjmsg.get(position).getBaddress());
        Glide.with(context).load(sjmsg.get(position).getBlogopic()).into(holder.iv_product_image);
        return convertView;
    }
