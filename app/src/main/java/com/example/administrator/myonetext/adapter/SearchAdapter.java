...
public class SearchAdapter extends BaseAdapter {
    ...
    private List<SearchBean.CpmsgBean> cpmsg;

    public SearchAdapter(Context context, List<SearchBean.CpmsgBean> cpmsg) {
    ...
    @Override
    public int getCount() {
        return cpmsg.size() == 0 ? 0 : cpmsg.size();
    }

    @Override
    public Object getItem(int position) {
        return cpmsg.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.tv_price.setText(cpmsg.get(position).getShopprice() + "");
        holder.tv_commodity_name.setText(cpmsg.get(position).getPname());
        holder.tv_comment.setText(cpmsg.get(position).getIcommentcnt() + "条评论数");
        holder.tv_address.setText(cpmsg.get(position).getPno());
        Glide.with(context).load(cpmsg.get(position).getPicurl()).into(holder.iv_product_image);
        return convertView;
    }
